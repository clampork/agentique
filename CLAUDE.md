...
- **`AgentState.swift`**—domain model. `AgentSlot` combines a `Workspace`, optional `AgentSession` (hook file), and optional `WorkspaceTag` (`cmux top`) into a `SlotState` (`working` / `ready` / `terminal`). Also holds `Palette` (brightness fractions and pulse timing—the tunables) and `CmuxColor`, a verbatim port of cmux's dark-appearance color handling so marks match what cmux renders.
